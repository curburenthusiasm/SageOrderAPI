...
# ----------------------------
MODEL = "llama3.1"  # choose a local model you have pulled
...
class InboxBot:
    ...
    def run_one_pass(self) -> Dict[str, Any]:
        """Run one processing pass of the inbox bot."""
        ...
        try:
            ...
            try:
                ...
                inbox_module.process_pass(ns, cfg)
                log_work_item(self.name, "Process inbox", "completed",
                             "Processed inbox emails and created drafts")
                return {"ok": True, "message": "Inbox processed successfully"}
            finally:
                try:
...
def delegate_to_leadtime_bot(task: str = "") -> Dict[str, Any]:
    """Delegate a lead time management task to the LeadTime Bot.

    The LeadTime Bot uses Claude API to manage customer lead time rules
    in the SQL Server Agent job scripts (shipping and production reports).
    """
    try:
        from leadtime_bot import delegate_leadtime_task
        log_work_item("LeadTimeBot", task, "started")
        result = delegate_leadtime_task(task)
        status = "completed" if result.get("ok") else "error"
        log_work_item("LeadTimeBot", task, status, result.get("message", result.get("error", "")))
        return result
    except ImportError:
        return {"ok": False, "error": "leadtime_bot module not found. Install: pip install anthropic pydantic"}
    except Exception as e:
        log_work_item("LeadTimeBot", task, "error", str(e))
        return {"ok": False, "error": str(e)}
...
TOOL_FUNCTIONS = {
    ...
    "delegate_to_leadtime_bot": delegate_to_leadtime_bot,
}
...
TOOLS = [
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "delegate_to_leadtime_bot",
            "description": "Delegate a lead time management task to the LeadTime Bot. Use this for requests about customer lead times, shipping deadlines, estimated ship dates, late thresholds, or SQL Server Agent job script updates.",
            "parameters": {
                "type": "object",
                "properties": {
                    "task": {
                        "type": "string",
                        "description": "Natural language description of the lead time task (e.g., 'Change Mattress Firm lead time to 4 days', 'List all customer lead times', 'Add new customer ACME with 5 day lead time')"
                    }
                },
                "required": ["task"]
            }
...
2. BOT MANAGEMENT: Supervise and delegate tasks to specialized bots:
   ...
   - LeadTimeBot: Manages customer lead time rules in SQL Server Agent job scripts
