...
password = keyring.get_password("JEF-SQL", "MAS_REPORTS")
if password is None:
    logging.error('Password not found in keyring')
    raise SystemExit('Could not get password from keyring')

# Establish a connection to the database
conn = pymssql.connect('JEF-SQL', 'MAS_REPORTS', password, 'MAS_JEF')
...
try:
    cursor.execute("""
    DELETE FROM MAS_JEF.dbo.SO_SalesOrderHeader WHERE BillToName = ''


DELETE d
        FROM MAS_JEF.dbo.SO_SalesOrderDetail AS d
        JOIN MAS_JEF.dbo.SO_SalesOrderHeader AS h ON d.SalesOrderNo = h.SalesOrderNo
